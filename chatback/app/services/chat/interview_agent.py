...
class InterviewAgent:
    ...
    def _get_user_info(self) -> dict:
        ...
        try:
            user_info = self.redis_client.get(f"interview:user_info_{self.session_id}")
            if user_info:
                return json.loads(user_info)
            logger.warning(f"No user info found for session {self.session_id}")
            return {}
        except Exception as e:
...
Is there anything else you'd like to add before we proceed with the document generation?"""
    ...
    async def save_interview_responses(self, chat_name: str) -> str:
        """Save interview responses to a markdown file in user's chat directory."""
        try:
            ...
            if not os.path.exists(self.template_path):
                ...
                raise FileNotFoundError(
                    f"Required template file not found. "
                    f"Please ensure it exists at {self.template_path}"
                )
            
            # Check if template is readable
            if not os.access(self.template_path, os.R_OK):
                logger.error(f"Template file is not readable: {self.template_path}")
                raise PermissionError(
                    f"Cannot read template file. "
                    f"Please check file permissions at {self.template_path}"
                )
            ...
            logger.info(f"Reading template from: {self.template_path}")
            ...
            with open(self.template_path, 'r') as template_file:
                try:
                    ...
                    logger.error(f"Failed to decode template file: {str(e)}")
                    raise ValueError(
                        f"Template file has invalid encoding. "
            ...
            if not os.access(os.path.dirname(filepath), os.W_OK):
                logger.error(f"Cannot write to directory: {os.path.dirname(filepath)}")
                raise PermissionError(
                    f"Cannot write to output directory. "
                    f"Please check permissions at {os.path.dirname(filepath)}"
                )
            ...
            with open(filepath, 'w') as f:
                try:
                    ...
                    raise IOError(f"Failed to save interview responses: {str(e)}")
