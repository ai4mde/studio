...
def _parse_design_md_to_tokens(content: str) -> dict:
    ...
    if palette_match:
        ...
        for key, hex_val in rows:
            k = key.strip().lower()
            if k == "primary":
                tokens["accent.hex"] = hex_val
            elif k == "background":
                tokens["page.body.bg_hex"] = hex_val
            elif k == "surface":
                tokens["region.main.bg_hex"] = hex_val
            elif k == "border":
...
def _build_activity_diagrams(system_data: dict) -> list:
    ...
    for diagram in diagrams:
        ...
        for node in raw_nodes:
            ...
            nodes.append({"id": str(node.get("id")), "cls_ptr": cls_id, "cls": classifier.get("data", {}), "data": node.get("data", {})})
            if cls_id:
...
def _workflow_plan(system_data: dict, actor_id: str | None, actor_name: str | None = None) -> list:
    ...
    for diagram in system_data.get("activity_diagrams", []):
        ...
        for node in diagram.get("nodes", []):
            ...
            next_action_ids = [target for target in outgoing.get(str(node.get("id")), []) if (nodes.get(target, {}).get("cls") or {}).get("type") == "action"]
...
def _ensure_workflow_pages(pages: list, sections: list, workflow_steps: list) -> tuple[list, list]:
    ...
    for step in workflow_steps:
        ...
        if not page:
            page = {"id": step["page_id"], "name": step["page_name"], "primary_model": "", "type": {"value": "activity", "label": "Activity"}, "action": {"value": node_id, "label": step["activity_node_name"]}, "sections": [], "category": None}
            pages.append(page)
...
def update_interface_data(interface_id: str, data: dict) -> str:
    try:
        ...
        current = resp.json()
        payload = {"id": interface_id, "name": current["name"], "description": current["description"], "system_id": current["system"], "actor_id": current["actor"], "data": data}
...
def validate_and_save_candidate(interface_id, candidate_index, name, description, pages, sections, tokens=None, styling=None):
    try:
        if styling:
            ...
            alias_map = {"accent_color": "accentColor", "background_color": "backgroundColor", "text_color": "textColor", "selected_style": "selectedStyle"}
