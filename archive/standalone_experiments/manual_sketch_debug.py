...
from llm.pipeline_profiles import PipelineProfile
...
def _print_requested_artifacts(args: argparse.Namespace, bundle: Dict[str, Any]) -> None:
    ...
    if args.show_topology_artifact and stage_artifacts.get("topology_artifact") is not None:
        _print_json_section("TOPOLOGY_ARTIFACT", stage_artifacts["topology_artifact"])
    if args.show_topology_artifact_prompt and stage_artifacts.get("topology_artifact_prompt") is not None:
        _print_json_section("TOPOLOGY_ARTIFACT_PROMPT", stage_artifacts["topology_artifact_prompt"])
    if args.show_semantic_plan and stage_artifacts.get("semantic_plan") is not None:
        _print_json_section("SEMANTIC_PLAN", stage_artifacts["semantic_plan"])
    if args.show_semantic_prompt and stage_artifacts.get("semantic_prompt") is not None:
        _print_json_section("SEMANTIC_PROMPT", stage_artifacts["semantic_prompt"])
    if args.show_sketch_prompt and stage_artifacts.get("sketch_prompt") is not None:
        _print_json_section("SKETCH_PROMPT", stage_artifacts["sketch_prompt"])
    ...
    if args.show_deterministic_sketch and stage_artifacts.get("deterministic_sketch") is not None:
        _print_json_section("DETERMINISTIC_SKETCH", stage_artifacts["deterministic_sketch"])
    ...
    if args.show_compiled_activity_graph and stage_artifacts.get("compiled_activity_graph") is not None:
        _print_json_section("COMPILED_ACTIVITY_GRAPH", stage_artifacts["compiled_activity_graph"])
...
def _write_failure_artifact(*, process_text: str, run_index: int, artifacts: Dict[str, Any]) -> str:
    output_dir = Path("/private/tmp/manual-sketch-debug-failures")
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / f"sketch_failure_{uuid.uuid4().hex}_run_{run_index}.json"
    payload = {
...
def _print_failure_summary(
    ...
    *,
    ...
    for failure in failures:
        ...
        if failure.get("failure_artifact_path"):
            print(f"failure_artifact: {failure['failure_artifact_path']}")
...
def main() -> None:
    ...
    parser.add_argument("--show-topology-artifact", action="store_true", help="Print the generated topology artifact JSON when guidance is enabled.")
    parser.add_argument("--show-topology-artifact-prompt", action="store_true", help="Print the topology artifact prompt when guidance is enabled.")
    parser.add_argument("--show-semantic-plan", action="store_true", help="Print the generated semantic plan JSON when semantic_deterministic is enabled.")
    parser.add_argument("--show-semantic-prompt", action="store_true", help="Print the semantic planner prompt when semantic_deterministic is enabled.")
    parser.add_argument("--show-sketch-prompt", action="store_true", help="Print the rendered Sketch Planner prompt.")
    parser.add_argument("--show-deterministic-sketch", action="store_true", help="Print the deterministic ActivitySketch assembled from topology and semantics.")
    parser.add_argument("--show-compiled-activity-graph", action="store_true", help="Print the compiled ActivityGraph before final validation output.")
    ...
    parser.add_argument("--use-topology-artifact-guidance", action="store_true", help="Generate a topology artifact first and provide it as guidance to the Sketch Planner.")
    parser.add_argument(
        "--import",
        dest="do_import",
        action="store_true",
    ...
    if args.show_all_stages:
        ...
        args.show_topology_artifact = True
        args.show_topology_artifact_prompt = True
        args.show_semantic_plan = True
        args.show_semantic_prompt = True
        args.show_sketch_prompt = True
        ...
        args.show_deterministic_sketch = True
        args.show_graph = True
        args.show_compiled_activity_graph = True
            ...
            try:
                if args.use_experimental_compiler:
                    bundle = debug_model_activity_with_experimental_compiler(
                        ...
                        use_topology_artifact_guidance=args.use_topology_artifact_guidance,
                    )
                else:
                ...
                if isinstance(debug_artifacts, dict):
                    failure_artifact_path = _write_failure_artifact(
                        process_text=process_text,
                        run_index=run_index,
                ...
                if failure_artifact_path:
                    print(f"failure_artifact: {failure_artifact_path}")
                continue
