...
from .control_flow_scoring import aggregate_control_flow_scores, score_control_flow_case
...
def main() -> None:
    ...
    report = {
        "scope": {
            "purpose": "Control-flow scorer validation over frozen Action decisions",
            "reviewed_cases": list(REVIEWED_CASE_SOURCES),
            "formal_full_47_case_evaluation": False,
            "control_structure_scoring": False,
        },
        "cases": [score.to_dict() for score in case_scores],
        "aggregate": aggregate_control_flow_scores(case_scores).to_dict(),
        "accounting_invariants": {
            score.case_id: {
                "TP_plus_FN_equals_reference_flows": (
                    score.TP + score.FN == score.reference_flow_count
                ),
                "TP_plus_FP_equals_generated_flows": (
                    score.TP + score.FP == score.generated_flow_count
                ),
            }
            for score in case_scores
        },
    }
    output_path = base / "control_flow_scorer_validation.json"
