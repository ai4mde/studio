...
REFERENCE_RELATIVE_DELTA = 0.10
CONTEXT_SIMILARITY_TOLERANCE = 0.01
COMPATIBILITY_VERSION = "action-compatibility-v3"
COMPATIBILITY_FILENAME = "compatibility_v3.json"
...
class Compatibility:
    ...
    generated_operation: str | None = None


def _compatibility_taxonomy() -> dict[str, Any]:
    path = Path(__file__).with_name(COMPATIBILITY_FILENAME)
    return json.loads(path.read_text(encoding="utf-8"))


def compatibility_veto(reference_label: str, generated_label: str) -> Compatibility:
    """Apply only explicit, taxonomy-backed semantic conflicts."""
    taxonomy = _compatibility_taxonomy()
    families = taxonomy["operation_families"]
    ref_tokens, gen_tokens = reference_label.split(), generated_label.split()
    ref_operations = [(token, families[token]) for token in ref_tokens if token in families]
    gen_operations = [(token, families[token]) for token in gen_tokens if token in families]
    ref_primary = ref_operations[0] if ref_operations else None
    gen_primary = gen_operations[0] if gen_operations else None
    incompatible = {frozenset(pair) for pair in taxonomy["incompatible_family_pairs"]}
    if ref_primary and gen_primary and frozenset((ref_primary[1], gen_primary[1])) in incompatible:
        return Compatibility(False, f"incompatible_primary_operations:{ref_primary[1]}:{gen_primary[1]}",
                             ref_primary[0], gen_primary[0])
    negatives = set(taxonomy["negative_tokens"])
    if bool(negatives & set(ref_tokens)) != bool(negatives & set(gen_tokens)):
        return Compatibility(False, "explicit_polarity_conflict",
                             ref_primary[0] if ref_primary else None,
                             gen_primary[0] if gen_primary else None)
    concrete = set(taxonomy["concrete_objects"])
    ref_objects, gen_objects = concrete & set(ref_tokens), concrete & set(gen_tokens)
    if ref_objects and gen_objects and ref_objects.isdisjoint(gen_objects):
        return Compatibility(False, "explicit_concrete_object_conflict",
                             ref_primary[0] if ref_primary else None,
                             gen_primary[0] if gen_primary else None)
    return Compatibility(True, "no_explicit_conflict",
                         ref_primary[0] if ref_primary else None,
                         gen_primary[0] if gen_primary else None)
...
class SentenceTransformerSimilarity:
    ...
    def __init__(self, model_name: str, revision: str, cache_folder: str) -> None:
        ...
        if not snapshot.is_dir():
            raise RuntimeError(f"Pinned Action model snapshot is unavailable: {snapshot}")
...
class ActionResult:
    ...
    def metric_defined(self) -> bool:
        return bool(self.counts.tp or self.counts.fp or self.counts.fn)
...
def _lexicographic_assignment(
    scores: list[list[float]], eligible: list[list[bool]]
) -> list[tuple[int, int]]:
    ...
    objective = [
        [cardinality_unit + score if eligible[i][j] else 0.0 for j, score in enumerate(row)]
        for i, row in enumerate(scores)
    ]
    return [(i, j) for i, j in _maximum_assignment(objective) if eligible[i][j]]


def _adjacent(graph: EvalGraph, left: str, right: str) -> bool:
    return right in graph.outgoing[left] or left in graph.outgoing[right]


def _compound(label: str | None) -> bool:
    tokens = set((label or "").split())
    return bool(tokens & {"and", "then", "plus"}) or "/" in (label or "")


def _near_duplicate(left: str | None, right: str | None) -> bool:
    left_tokens, right_tokens = set((left or "").split()), set((right or "").split())
...
def evaluate_actions(
    ...
    *,
    ...
    compatibility = [
        [compatibility_veto(ref[i].label or "", gen[j].label or "") for j in range(len(gen))]
        for i in range(len(ref))
    ]
    eligible = [
        [scores[i][j] >= threshold and compatibility[i][j].compatible for j in range(len(gen))]
        for i in range(len(ref))
    ]
    strongest_eligible = [
        max((scores[i][j] for j in range(len(gen)) if eligible[i][j]), default=None)
        for i in range(len(ref))
    ]
    retained = [
        [
            eligible[i][j]
            and strongest_eligible[i] is not None
            and scores[i][j] >= strongest_eligible[i] - REFERENCE_RELATIVE_DELTA
            for j in range(len(gen))
        ]
        for i in range(len(ref))
    ]
    pairs = _lexicographic_assignment(scores, retained)
    ...
    while remaining:
        seed = min(remaining)
        group = {index for index in remaining if _near_duplicate(ref[seed].label, ref[index].label)}
        remaining -= group
    ...
    for indices in duplicate_groups:
        ...
        proposed = [(ref_index[left], gen_index[right]) for left, right in zip(ref_order, gen_order)]
        original_similarity = sum(scores[i][j] for i, j in selected)
        proposed_similarity = sum(scores[i][j] for i, j in proposed)
        if (
            all(retained[i][j] for i, j in proposed)
            and original_similarity - proposed_similarity
            <= CONTEXT_SIMILARITY_TOLERANCE * len(proposed)
        ):
    ...
    for j in unmatched_gen:
        related = [
            (i, matched_j) for i, matched_j in pairs
            if eligible[i][j]
            and (_compound(ref[i].label) or _adjacent(generated, gen[matched_j].id, gen[j].id))
        ]
        if related:
    ...
    for i in unmatched_ref:
        related = [
            (matched_i, j) for matched_i, j in pairs
            if eligible[i][j]
            and (_compound(gen[j].label) or _adjacent(reference, ref[matched_i].id, ref[i].id))
        ]
        if related:
            ref_review[i].add("possible_merge_action")
            for pair in related:
                pair_reasons[pair].add("possible_merge_action")
    ...
    for node in ref:
        if node.id not in matched_ref:
            index = ref.index(node)
            compatible_scores = [
                scores[index][j] for j in range(len(gen)) if compatibility[index][j].compatible
            ]
            if strongest_eligible[index] is not None:
                filter_stage = "one_to_one_assignment"
            elif compatible_scores and max(compatible_scores) < threshold:
                filter_stage = "below_similarity_threshold"
            else:
                filter_stage = "no_compatible_candidate"
            items.append(AutomaticItem(case_id, candidate_id, "Action",
                evaluation_item_id(case_id, candidate_id, "action", "missing", node.id), "FN",
                {"reference_id": node.id, "review_triggers": sorted(ref_review[index])},
                {"reference_label": node.label, "reference_context": _local_context(reference, node.id),
                 ...
                 "matching_filter": {
                     "stage": filter_stage,
                     "threshold": threshold,
                     "delta_ref": REFERENCE_RELATIVE_DELTA,
                     "strongest_compatible_similarity": max(compatible_scores, default=None),
                     "strongest_eligible_similarity": strongest_eligible[index],
                 },
                 **ref_review_evidence[index]},
