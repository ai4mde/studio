...
if [ $PROJECT_NAME == "latest" ]; then
	...
	if test -d latest; then
		...
		printf '%s\n' "$PSOUT" | while IFS= read -r line; do
			if [[ $line == *"python3 manage.py runserver 127.0.0.1:8001"* ]]; then
...
if [ $PROJECT_NAME == "latest" ]; then
	if test -f ../previous/db.sqlite3; then
		...
		if [ $SAVE_IN_BETWEEN_RUNS == 1 ]; then
